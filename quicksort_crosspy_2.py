...
def scatter_2(splits, active_array, left_array, right_array):
    """
    :param xA, xB,  sliced crosspy arrays for the active element set
    :param splits, a host ndarray of how many left elements are in each partition 
    """
    ...
    for i in range(n_partitions):
        left_array[left_prefix[i]:left_prefix[i+1]
                   ] = active_array[size_prefix[i], size_prefix[i]+splits[i+1]]
        right_array[right_prefix[i]:right_prefix[i+1]
                    ] = active_array[size_prefix[i]+splits[i+1], size_prefix[i+1]]
