...
def test_parray_task():
    with Parla():
        ...
        async def main():
            ...
            @spawn(ts[1], placement=[gpu(1)], input=[(a, 0)])
            def task1():
            ...
            @spawn(ts[2], placement=[gpu(3)], inout=[(a, 0)])
            def task2():
            ...
            @spawn(ts[7], placement=[gpu(1)], input=[(c[0:2], 0)])
            def task7():
            ...
            @spawn(ts[8], placement=[gpu(3)], inout=[(c[0:2], 0)])
            def task8():
            ...
            @spawn(ts[10], placement=[gpu(2)], inout=[(c[0:2], 0)])
            def task10():
            ...
            @spawn(ts[11], dependencies=[ts[10]], placement=[gpu(3)], input=[(c[0:2], 0)])
            def task11():
                assert c[0:2].get_num_active_tasks(1) == 0
                assert c[0:2].get_num_active_tasks(2) == 0
            ...
            @spawn(ts[12], dependencies=[ts[11]], placement=[gpu(2)], inout=[(c[0:2], 0)])
            def task12():
                assert c[0:2].get_num_active_tasks(1) == 0
                assert c[0:2].get_num_active_tasks(2) == 0
            ...
            d = parray.asarray(d)
            @spawn(ts[13], placement=[gpu(3)], inout=[(d, 0)])
            def task13():
                assert gpu(3).query_mapped_resource(0) == 40
            await ts[13]

            assert gpu(3).query_mapped_resource(0) == 40
            d._auto_move(-1, do_write = True)
            assert gpu(3).query_mapped_resource(0) == 0

            @spawn(ts[14], placement=[gpu(2)], inout=[(d[0:2], 0)])
            def task14():
