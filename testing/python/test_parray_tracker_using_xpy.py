from parla import Parla, spawn, TaskSpace, parray as pa
        ...
        async def main():
            ...
            for i in range(0,4):
                with cp.cuda.Device(i):
                    a_list.append(cp.array([1, 2, 3, 4]))
                    test = pa.asarray(a_list[-1])
                    parray_list.append(test)
                    print("test:", test)
                    print("type:", type(test))
                    print("owner:", test._coherence.owner)
                    print("device owner:", test.device)
            '''
            x_cross = xp.array(parray_list, dim=0)
            ...
            @spawn(placement=[cuda(0), cuda(1), cuda(2), cuda(3)], input=[x_cross])
            def t1():
                print("test")
