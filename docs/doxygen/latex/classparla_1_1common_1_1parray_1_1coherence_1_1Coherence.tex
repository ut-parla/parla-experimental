...
\DoxyCodeLine{459 \textcolor{stringliteral}{            Or [ERROR] if this device has the last copy and `keep\_one\_copy` is false,  }}
\DoxyCodeLine{460 \textcolor{stringliteral}{            since eviction cannot be performed. }}
\DoxyCodeLine{461 \textcolor{stringliteral}{        "{}"{}"{}} }
\DoxyCodeLine{462         device\_local\_state = self.\_local\_states[device\_id] }
\DoxyCodeLine{463         operations = [] }
\DoxyCodeLine{464         evict\_last\_copy = \textcolor{keyword}{False} }
\DoxyCodeLine{465  }
\DoxyCodeLine{466         \textcolor{keywordflow}{if} device\_local\_state == self.INVALID: \textcolor{comment}{\# already evicted, do nothing }}
\DoxyCodeLine{467             operations.append(MemoryOperation.noop()) }
\DoxyCodeLine{468         \textcolor{keywordflow}{elif} device\_local\_state == self.SHARED: }
\DoxyCodeLine{469             \textcolor{keywordflow}{if} device\_id == self.owner:  \textcolor{comment}{\# has a chance this is the last copy }}
\DoxyCodeLine{470                 \textcolor{comment}{\# find new owner }}
\DoxyCodeLine{471                 new\_owner = \textcolor{keywordtype}{None} }
\DoxyCodeLine{472                 \textcolor{keywordflow}{for} device, state \textcolor{keywordflow}{in} self.\_local\_states.items(): }
\DoxyCodeLine{473                     \textcolor{keywordflow}{if} state == self.SHARED \textcolor{keywordflow}{and} device != device\_id:  \textcolor{comment}{\# should not include this device itself }}
\DoxyCodeLine{474                         new\_owner = device }
\DoxyCodeLine{475                         \textcolor{keywordflow}{break} }
\DoxyCodeLine{476  }
\DoxyCodeLine{477                 \textcolor{comment}{\# this device owns the last copy }}
\DoxyCodeLine{478                 \textcolor{keywordflow}{if} new\_owner \textcolor{keywordflow}{is} \textcolor{keywordtype}{None}: }
\DoxyCodeLine{479                     evict\_last\_copy = \textcolor{keyword}{True} }
\DoxyCodeLine{480                 \textcolor{keywordflow}{else}: }
\DoxyCodeLine{481                     self.owner = new\_owner }
\DoxyCodeLine{482         \textcolor{keywordflow}{else}:  \textcolor{comment}{\# Modified, this device owns the last copy }}
\DoxyCodeLine{483             evict\_last\_copy = \textcolor{keyword}{True} }
\DoxyCodeLine{484  }
\DoxyCodeLine{485         \textcolor{keywordflow}{if} evict\_last\_copy: }
\DoxyCodeLine{486             \textcolor{keywordflow}{if} keep\_one\_copy:  \textcolor{comment}{\# write back to CPU }}
\DoxyCodeLine{487                 \textcolor{keywordflow}{if} device\_id != CPU\_INDEX:               }
\DoxyCodeLine{488                     operations.extend(self.\_write\_back\_to(CPU\_INDEX, self.MODIFIED, on\_different\_device=\textcolor{keyword}{True}, this\_device\_id=device\_id)) }
\DoxyCodeLine{489                     \textcolor{comment}{\# special case, since `this\_device\_id` is set, \_write\_back will not evict this devic }}
\DoxyCodeLine{490                     \textcolor{comment}{\# need to do it manually }}
\DoxyCodeLine{491                     operations.append(MemoryOperation.evict(device\_id)) }
\DoxyCodeLine{492  }
\DoxyCodeLine{493                     self.owner = CPU\_INDEX }
\DoxyCodeLine{494                     self.\_local\_states[CPU\_INDEX] = self.MODIFIED }
\DoxyCodeLine{495                     self.\_is\_complete[device\_id] = \textcolor{keyword}{True} }
\DoxyCodeLine{496                 \textcolor{keywordflow}{else}: }
\DoxyCodeLine{497                     \textcolor{keywordflow}{return} [MemoryOperation.noop()] }
\DoxyCodeLine{498             \textcolor{keywordflow}{else}: }
\DoxyCodeLine{499                 \textcolor{comment}{\# do nothing and report error }}
\DoxyCodeLine{500                 \textcolor{keywordflow}{return} [MemoryOperation.error()] }
\DoxyCodeLine{501         \textcolor{keywordflow}{else}: }
\DoxyCodeLine{502             \textcolor{comment}{\# update states for eviction }}
\DoxyCodeLine{503             self.\_local\_states[device\_id] = self.INVALID }
\DoxyCodeLine{504             self.\_versions[device\_id] = -\/1 }
\DoxyCodeLine{505             self.\_is\_complete[device\_id] = \textcolor{keywordtype}{None} }
\DoxyCodeLine{506  }
\DoxyCodeLine{507             operations.append(MemoryOperation.evict(device\_id)) }
\DoxyCodeLine{508  }
\DoxyCodeLine{509         \textcolor{keywordflow}{return} operations }
...
\DoxyCodeLine{127             self.\_versions[device\_id] = -\/1 }
