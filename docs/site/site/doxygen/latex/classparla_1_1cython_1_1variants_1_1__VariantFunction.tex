...
\item 
def \mbox{\hyperlink{classparla_1_1cython_1_1variants_1_1__VariantFunction_a5c9311964dbc24b107f6d7e5fc64bc24}{variant}} (self, spec\+\_\+list, override=False, architecture=None, max\+\_\+amount=8)
...
Definition at line 72 of file variants.\+pyx.


\begin{DoxyCode}{0}
\DoxyCodeLine{72     \textcolor{keyword}{def }\_\_call\_\_(self, *args, **kwargs):}
\DoxyCodeLine{73         \textcolor{stringliteral}{"{}"{}"{}!}}
\DoxyCodeLine{74 \textcolor{stringliteral}{        @brief Call the function, dispatching to the appropriate variant.}}
\DoxyCodeLine{75 \textcolor{stringliteral}{        "{}"{}"{}}}
\DoxyCodeLine{76         local\_context = Locals.context }
\DoxyCodeLine{77         local\_devices = local\_context.get\_all\_devices()}
\DoxyCodeLine{78 }
\DoxyCodeLine{79         \textcolor{keywordflow}{if} len(local\_devices) == 0:}
\DoxyCodeLine{80             \textcolor{keywordflow}{return} self.\_default(*args, **kwargs)}
\DoxyCodeLine{81 }
\DoxyCodeLine{82         \textcolor{comment}{\# Construct a architecture specialization key from the local devices.}}
\DoxyCodeLine{83         spec\_key = tuple([d.architecture \textcolor{keywordflow}{for} d \textcolor{keywordflow}{in} local\_devices])}
\DoxyCodeLine{84 }
\DoxyCodeLine{85         \textcolor{comment}{\# Get the variant for the current specialization key.}}
\DoxyCodeLine{86         variant\_f = self.get\_variant(spec\_key)}
\DoxyCodeLine{87 }
\DoxyCodeLine{88         \textcolor{comment}{\# Call the variant.}}
\DoxyCodeLine{89         \textcolor{keywordflow}{return} variant\_f(*args, **kwargs)}
\DoxyCodeLine{90 }
\DoxyCodeLine{91 }
...
Definition at line 69 of file variants.\+pyx.


\begin{DoxyCode}{0}
\DoxyCodeLine{69     \textcolor{keyword}{def }\_\_repr\_\_(self):}
\DoxyCodeLine{70         \textcolor{keywordflow}{return} \textcolor{stringliteral}{"{}\{f\} specialized to \{targets\}>"{}}.format(f=repr(self.\_default)[:-\/1], targets=tuple(self.\_variants.\mbox{\hyperlink{namespaceparla_1_1cython_1_1tasks_a83206470aa63f6e6fb8614d30dcbbe74}{keys}}()))}
\DoxyCodeLine{71 }
...
\begin{DoxyCode}{0}
\DoxyCodeLine{63     \textcolor{keyword}{def }get\_variant(self, spec\_key):}
\DoxyCodeLine{64         \textcolor{stringliteral}{"{}"{}"{}!}}
\DoxyCodeLine{65 \textcolor{stringliteral}{        @brief Get the variant for a given specialization key.}}
\DoxyCodeLine{66 \textcolor{stringliteral}{        "{}"{}"{}}}
\DoxyCodeLine{67         \textcolor{keywordflow}{return} self.\_variants.get(spec\_key, self.\_default)}
\DoxyCodeLine{68     }
...
\end{center}
\end{figure}
\mbox{\Hypertarget{classparla_1_1cython_1_1variants_1_1__VariantFunction_a5c9311964dbc24b107f6d7e5fc64bc24}\label{classparla_1_1cython_1_1variants_1_1__VariantFunction_a5c9311964dbc24b107f6d7e5fc64bc24}} 
...
{\footnotesize\ttfamily def parla.\+cython.\+variants.\+\_\+\+Variant\+Function.\+variant (\begin{DoxyParamCaption}\item[{}]{self,  }\item[{}]{spec\+\_\+list,  }\item[{}]{override = {\ttfamily False},  }\item[{}]{architecture = {\ttfamily None},  }\item[{}]{max\+\_\+amount = {\ttfamily 8} }\end{DoxyParamCaption})}
...
\hline
{\em architecture} & The type of device that the variant is defined on (used to specify a range of valid configurations) \\
\hline
{\em max\+\_\+amount} & If using architecture this will specify the configurations as valid for (1, max\+\_\+amount) architecture devices \\
\hline
\end{DoxyParams}


Definition at line 32 of file variants.\+pyx.


\begin{DoxyCode}{0}
\DoxyCodeLine{32     \textcolor{keyword}{def }variant(self, spec\_list, override=False, architecture=None, max\_amount=8):}
\DoxyCodeLine{33         \textcolor{stringliteral}{"{}"{}"{}!}}
\DoxyCodeLine{34 \textcolor{stringliteral}{        @brief Decorator to declare a variant of this function for a specific architecture.}}
\DoxyCodeLine{35 \textcolor{stringliteral}{}}
\DoxyCodeLine{36 \textcolor{stringliteral}{        @param spec\_list A list of architectures to specialize this function for. Can be a single architecture, or a list of architectures. Each architecture can be a tuple of architectures to specialize for a multidevice configuration.}}
\DoxyCodeLine{37 \textcolor{stringliteral}{        @param override If true, allow overriding an existing variant for one of the given architectures.}}
\DoxyCodeLine{38 \textcolor{stringliteral}{        @param architecture The type of device that the variant is defined on (used to specify a range of valid configurations)}}
\DoxyCodeLine{39 \textcolor{stringliteral}{        @param max\_amount If using architecture this will specify the configurations as valid for (1, max\_amount) architecture devices}}
\DoxyCodeLine{40 \textcolor{stringliteral}{        "{}"{}"{}}}
\DoxyCodeLine{41 }
\DoxyCodeLine{42         \textcolor{keywordflow}{if} architecture \textcolor{keywordflow}{is} \textcolor{keywordflow}{not} \textcolor{keywordtype}{None}:}
\DoxyCodeLine{43             \textcolor{keywordflow}{if} spec\_list \textcolor{keywordflow}{is} \textcolor{keywordflow}{not} \textcolor{keywordtype}{None}:}
\DoxyCodeLine{44                 \textcolor{keywordflow}{raise} VariantDefinitionError(\textcolor{stringliteral}{"{}Cannot specify both architecture and spec\_list."{}})}
\DoxyCodeLine{45             spec\_list = [architecture*i \textcolor{keywordflow}{for} i \textcolor{keywordflow}{in} range(1, max\_amount+1)]}
\DoxyCodeLine{46 }
\DoxyCodeLine{47         \textcolor{keywordflow}{if} \textcolor{keywordflow}{not} isinstance(spec\_list, list):}
\DoxyCodeLine{48             spec\_list = [spec\_list]}
\DoxyCodeLine{49         }
\DoxyCodeLine{50         \textcolor{keywordflow}{if} any(t \textcolor{keywordflow}{in} self.\_variants \textcolor{keywordflow}{for} t \textcolor{keywordflow}{in} spec\_list) \textcolor{keywordflow}{and} \textcolor{keywordflow}{not} override:}
\DoxyCodeLine{51             \textcolor{keywordflow}{raise} VariantDefinitionError(\textcolor{stringliteral}{"{}Variant already exists for one of the given specialization targets."{}})}
\DoxyCodeLine{52 }
\DoxyCodeLine{53         \textcolor{keyword}{def }variant(f):}
\DoxyCodeLine{54             \textcolor{keywordflow}{for} t \textcolor{keywordflow}{in} spec\_list:}
\DoxyCodeLine{55                 \textcolor{keywordflow}{if} \textcolor{keywordflow}{not} isinstance(t, tuple):}
\DoxyCodeLine{56                     t = (t,)}
\DoxyCodeLine{57                 self.\_variants[t] = f}
\DoxyCodeLine{58             \textcolor{keywordflow}{return} f}
\DoxyCodeLine{59         }
\DoxyCodeLine{60         variant.\_\_name\_\_ = \textcolor{stringliteral}{"{}\{\}.variant"{}}.format(self.\_default.\_\_name\_\_)}
\DoxyCodeLine{61         \textcolor{keywordflow}{return} variant}
\DoxyCodeLine{62 }
