...
class Task:
    ...
    def instantiate(self, dependencies=None, list_of_dev_reqs=[], constraints=None, priority=None, runahead=SyncType.BLOCKING):
...
class ComputeTask(Task):
    ...
    def instantiate(self, function, args, dependencies=None, constraints=None, dataflow=None, priority=0, runahead=SyncType.NONE):
        ...
        super().instantiate(dependencies=dependencies, constraints=constraints, priority=priority, runahead=runahead)
...
class TaskEnvironment:
    ...
    def __init__(self, environment_list, blocking=False):
        ...
        for env in environment_list:
            print("env: ", env, environment_list, flush=True)
            for dev in env.devices:
                print("Device: ", dev, dev.architecture, flush=True)
                self.device_list.append(dev)
                print("Device List: ", self.device_list, flush=True)
                self.device_dict[dev.architecture].append(dev)
                print("Device Dict: ", self.device_dict, flush=True)

            print("out of loop", self._global_device_ids, env.device_ids, flush=True)
            self._global_device_ids  = self._global_device_ids.union(env.device_ids)
            self.env_list.append(env)

        print("FINISHED INITIALIZING ENVIRONMENT", self.env_list, self._global_device_ids, flush=True)
...
class TerminalEnvironment(TaskEnvironment):
    def  __init__(self,  device, blocking=False):
        ...
        print("Created: ", self, self._global_device_ids, flush=True)
