...
inline void launch_task_callback(launchfunc_t func, void* scheduler, void* task, void* worker) {
...
*/
template<typename T>
class InnerResourcePool{
    /*TODO : This is a mock placeholder for now. Used to test the scheduler and settle on API. Assumes setup happens sequentially and is externally thread-safe.
    *      : Internal representation should be a map of strings to types.
    *      : This should be able to subtract, check, and add a resource to the bool in an internally thread-safe manner.
    *      : This should be able to do the above for all resources in another pool
    *      : This only handles a single type of resource. Ideal should be able to handle multiple types like the Python dictionary :( 
    *      : (Needed) Multilevel dispatch for mixed types. e.g. threads->int, vcus->rational. constexpr if?
    *      : (Optional) Be able to compare pools of different types
    */
    public:
        std::atomic<T> vcus {0};
...
*   All scheduling logic should be handled by these after creation until launched by the Python callback
*/
class InnerTask {
    ...
    public:
        ...
        /* Add a list of dependencies to the task and process them. For external use.*/
        ...
        //void add_dependents(std::vector<bool> result, std::vector<InnerTask*>& tasks);
...
class InnerWorker {
    ...
    public:
        ...
        void set_py_worker(void *worker){
            this->py_worker = worker;
        };
...
namespace Scheduler {
    ...
    enum State {spawned, mapped, reserved, ready, launch, running, complete, failed};
    ...
    class Status{
        ...
        public:
            ...
            void print(){
                std::cout << "Scheduler Status: (" << this->status[0] << ", " << this->status[1] << ", " << this->status[2] << ", " << this->status[3] << ", " << this->status[4] << ", " << this->status[5] << ", " << this->status[6] << ", " << this->status[7] << ")" << std::endl;
            }
    };
...
class InnerScheduler {
    ...
    public:
        ...
        /* Resource Pool */
        InnerResourcePool<float>* resources; //TODO: Dummy class, needs complete rework with devices

        /* Active task counter (thread-safe) */
        std::atomic<int> num_active_tasks;
        ...
        void set_resources(std::string resource_name, float resource_value); //TODO: Dummy function, needs complete rework with devices
        ...
        /* Complete all task finalization. Notify Dependents / Release Resources / Worker Enqueue */
        void task_cleanup(InnerWorker* worker, InnerTask* task, int state);

        /* Get number of active tasks. A task is active if it is spawned but not complete */
        int get_num_active_tasks();

        /* Increase number of active tasks */
        void increase_num_active_tasks();

        /* Decrease number of active tasks. If zero tasks are active, stop the scheduler */
        void decrease_num_active_tasks();

        /* Get number of running tasks. A task is running if is Python task has been assigned and the task is not complete*/
        int get_num_running_tasks();

        /* Get number of ready tasks. A task is ready if its dependencies has been dispatched to a hardware queue or are complete */
