...
void LocalityLoadBalancingMappingPolicy::calc_score_devplacement(
    InnerTask *task, const std::shared_ptr<DeviceRequirement> &dev_placement_req,
  ...
  *average_score = 0;
  ...
  for (DevID_t did = 0; did < placement_reqs_vec.size(); ++did) {
    ...
    if (placement_req->is_dev_req()) {
      ...
      this->calc_score_devplacement(task, dev_req,
          num_total_mapped_tasks, &score);
      ...
      this->calc_score_archplacement(task, arch_req,
          num_total_mapped_tasks, dev_req, &score);
    }
