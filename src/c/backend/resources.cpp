...
// I don't think other would EVER need to be atomic I'm really just doing this
// for convienence in the mockup.

template <typename T> void InnerResourcePool<T>::set(std::string, T value) {
  this->vcus.store(value);
}

template <typename T> T InnerResourcePool<T>::get(std::string) {
...
bool InnerResourcePool<T>::check_greater(InnerResourcePool<J> &other) {
  ...
  auto a = this->vcus.load();
  auto b = other.vcus.load();
