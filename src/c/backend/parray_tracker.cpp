...
void PArrayTracker::release_parray(const InnerPArray& parray, Device* device) {
  ...
  if (this->managed_parrays_[dev_global_id][parray.parent_id] == true) {
    ...
    // the slice, as it is set at __init__() of the slice PArray and can be seen,
    // but deallocates the memory byte size of its parent PArray.
    // This makes the mapped memory size counter to a negative number, which
    // we do not prefer.
    // Instead, we set 0 if the counter could be a negative number.
