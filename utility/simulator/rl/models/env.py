...
def create_gcn_task_workload_state(
    node_id_offset, target_task: SimulatedTask,
    ...
    lst_node_features.append(
        create_task_workload_state(target_task, devices, taskmap))
    ...
    for dependency_id in target_task.dependencies:
        ...
        lst_node_features.append(
            create_task_workload_state(dependency, devices, taskmap))
        node_id_offset += 1
    for dependent_id in target_task.dependents:
        ...
        lst_node_features.append(
            create_task_workload_state(dependent, devices, taskmap))
        node_id_offset += 1
...
def create_task_workload_state(
    target_task: SimulatedTask, devices: List,
...
def create_device_load_state(target_task: SimulatedTask, devices: List,
    ...
    for device in devices:
        ...
        idx += device_feature_dim
    return current_state

def create_state(target_task: SimulatedTask, devices: List, taskmap: Dict,
                 reservable_tasks: Dict, launchable_tasks: Dict,
                 launched_tasks: Dict):
    """
    ...
    """
    current_device_load_state = create_device_load_state(
        target_task, devices, reservable_tasks, launchable_tasks,
        launched_tasks)
    edge_index, current_workload_features = create_gcn_task_workload_state(
        0, target_task, devices, taskmap)
    return current_device_load_state, edge_index, current_workload_features

def create_next_state(current_device_load_state, edge_index,
     ...
     num_reservable_task_offset = action * device_feature_dim
