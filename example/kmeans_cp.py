...
def fit_xp(X, n_clusters, max_iter):
    ...
    for _ in range(max_iter):
        ...
        pred = xp.argmin(distances, axis=1)
...
def run(gpuid, n_clusters, num, max_iter, use_custom_kernel, output):
    samples = numpy.random.randn(num, 10)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpu-id', '-g', default=0, type=int,
                        help='ID of GPU.')
    parser.add_argument('--n-clusters', '-n', default=10, type=int,
                        help='number of clusters')
    parser.add_argument('--num', default=1250000, type=int,
                        help='number of samples')
    parser.add_argument('--max-iter', '-m', default=20, type=int,
